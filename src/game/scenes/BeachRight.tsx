...
export class BeachRight extends Scene {
  ...
  preload() {
    ...
    this.load.image('sign_left', './assets/arrow_left.png');
  }
  ...
  create() {
    ...
    this.add.image(100, GAME_CONFIG.WALK_PATH_Y, 'sign_left').setDisplaySize(250, 250).setDepth(1);

    const itemsConfig = GAME_CONFIG.ITEMS.filter(item => 
      item.location === "beach_right" && !this.gameData.includes(item.id)
    );
